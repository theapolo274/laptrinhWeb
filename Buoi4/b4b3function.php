<?php
  ...
  // Retrieve and sanitize values from cookies
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($firstname) || empty($lastname) || empty($email) || empty($id) || empty($pay) || empty($upload) ||empty($type)) {
            ...
            $_SESSION["pay_for"] = $pay;
            $_SESSION["fileToUpload"] = $upload;
            $_SESSION["content"] = $type;
            header("Location: b4b3hienthi.php");
            exit();
        }
    }
